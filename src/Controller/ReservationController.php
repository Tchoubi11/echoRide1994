...
class ReservationController extends AbstractController
{
    ...
    public function participate(
        ...
        $commissionPlatform = 2;
        $prixTotal = $ride->getPrixPersonne() + $commissionPlatform;
        ...
        if ($request->isMethod('POST')) {
            if ($session->get('confirm_' . $rideId)) {
                ...
                try {
                    ...
                    if (!$success) {
                        throw new \Exception('Erreur lors du traitement des crédits du passager.');
                    }

                    $platformId = 0;
                    $creditService->addCredits($platformId, $commissionPlatform);

                    $chauffeur = $ride->getDriver();
                    if (!$chauffeur) {
                        throw new \Exception('Chauffeur introuvable pour ce covoiturage.');
                    }
                    $creditService->addCredits($chauffeur->getId(), $ride->getPrixPersonne());
