    ...
    public function validateReservations(
        ...
        foreach ($reservations as $reservation) {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if ($reservation->getProblemeSignale()) {
                    $notificationService->notifyAdminOfIssue($reservation);
                    $this->addFlash('warning', 'Votre problème a été transmis. Le crédit du chauffeur est temporairement bloqué.');
                ...
                $allConfirmed = true;
                foreach ($covoiturage->getReservations() as $res) {
                    if (!$res->getAConfirmeParticipation()) {
                        $allConfirmed = false;
                        break;
                    }
                }
